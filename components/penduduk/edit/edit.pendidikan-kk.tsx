...
export const EditPendidikanKk = ({ initialData, nik }: Props) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      //   const formatString = values.pendidikan_kk.toUpperCase();
      //   await axios.patch(`/api/penduduk/${nik}`, {
      //     pendidikan_kk: formatString,
      //   });
      //   toast.success("Pendidikan Dalam KK berhasil diubah");
      //   toggleEdit();
      //   router.refresh();
  ...
  return (
    <div className="mt-6 border bg-slate-100 rounded-md p-4">
      ...
      {!isEditing && <div className={cn("text-sm mt-2", !initialData.pendidikan_kk && "text-slate-500 italic")}>{!initialData.pendidikan_kk ? "Tidak diketahui" : initialData.pendidikan_kk}</div>}
      {isEditing && (
        <Form {...form}>
