...
const formSchema = z.object({
  status_duk: z.string().min(1),
});

export const EditStatusDuk = ({ initialData, nik }: Props) => {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      status_duk: initialData.status_duk || "",
    },
  });
  ...
  return (
    <div className="mt-6 border bg-slate-100 rounded-md p-4">
      ...
      {!isEditing && <div className={cn("text-sm mt-2", !initialData.status_duk && "text-slate-500 italic")}>{!initialData.status_duk ? "Tidak diketahui" : initialData.status_duk}</div>}
      {isEditing && (
        <Form {...form}>
