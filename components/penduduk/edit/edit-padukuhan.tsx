...
export const EditPadukuhan = ({ initialData, nik }: Props) => {
  ...
  return (
    <div className="mt-6 border bg-slate-100 rounded-md p-4">
      ...
      {!isEditing && <div className={cn("text-sm mt-2", !initialData.padukuhan && "text-slate-500 italic")}>{!initialData.padukuhan ? "Tidak diketahui" : initialData.padukuhan}</div>}
      {isEditing && (
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <Input disabled={isSubmitting} placeholder="Masukan Padukuhan" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
