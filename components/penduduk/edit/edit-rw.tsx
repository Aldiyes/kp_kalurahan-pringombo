...
export const EditRw = ({ initialData, nik }: Props) => {
  ...
  return (
    <div className="mt-6 border bg-slate-100 rounded-md p-4">
      ...
      {!isEditing && <div className={cn("text-sm mt-2", !initialData.rw && "text-slate-500 italic")}>{!initialData.rw ? "Tidak diketahui" : initialData.rw}</div>}
      {isEditing && (
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <Input type="number" disabled={isSubmitting} placeholder="Masukan nomor RW" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
