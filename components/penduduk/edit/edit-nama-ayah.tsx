...
export const EditNamaAyah = ({ initialData, nik }: Props) => {
	...
	const onSubmit = async (values: z.infer<typeof formSchema>) => {
		try {
			values.nama_ayah = values.nama_ayah.toUpperCase();
			startTransition(() => {
		...
		<div className="mt-6 border bg-slate-100 rounded-md p-4">
			...
			{!isEditing && (
				<div
					className={cn(
						'text-sm mt-2',
						!initialData.nama_ayah && 'text-slate-500 italic'
					)}
				>
					{!initialData.nama_ayah ? 'Tidak diketahui' : initialData.nama_ayah}
				</div>
			)}
			{isEditing && (
				<Form {...form}>
